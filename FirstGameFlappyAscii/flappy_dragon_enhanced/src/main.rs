...
struct Player {
    ...
    y: f32,
    velocity: f32,
    frame: usize, // Usize for array indexing
}
...
impl State {
    fn new() -> Self {
        State {
            player: Player::new(5, (SCREEN_WIDTH as f32) / 2.0),
            frame_time: 0.0,
    ...
    fn restart(&mut self) {
        self.player = Player::new(5, (SCREEN_WIDTH as f32) / 2.0);
        self.frame_time = 0.0;
...
fn main() -> BError {
    let context = BTermBuilder::new()
        .with_font("../resources/flappy32.png", 32, 32) // 32 pixel for each char, we have a 512x512 spirit sheet, a 16x16 matrix
